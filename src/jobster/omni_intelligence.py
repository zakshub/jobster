...
def build_job_explanation(profile: CareerProfile, store: JobsterStore, job_id: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "learnable_gaps": list(evaluation.get("learnable_gaps") or []),
...
def build_career_graph(profile: CareerProfile, store: JobsterStore, *, job_limit: int = 24) -> dict[str, Any]:
    nodes: list[dict[str, Any]] = []
    edges: list[dict[str, str]] = []
    person_id = f"person:{profile.profile_id}"
    nodes.append({"id": person_id, "type": "person", "label": profile.display_name, "meta": profile.headline or "Career profile", "weight": 10})

    for index, title in enumerate(profile.target_titles):
        node_id = f"role:{index}:{title}"
        nodes.append({"id": node_id, "type": "target_role", "label": title, "meta": "Target role", "weight": 8})
    ...
    for index, experience in enumerate(profile.experiences):
        ...
        nodes.append({"id": node_id, "type": "experience", "label": experience.company, "meta": experience.title, "weight": 6})
        edges.append({"source": person_id, "target": node_id, "type": "worked_at"})

    jobs = [
        item for item in store.list_job_summaries(limit=300)
        if not item.get("dismissed") and item.get("decision") in _PRIORITY_DECISIONS
    ]
    ...
    for job in jobs[:job_limit]:
        ...
        if company_id not in seen_companies:
            ...
            nodes.append({"id": company_id, "type": "company", "label": company, "meta": "Observed company", "weight": 5})
        job_id = f"job:{job['id']}"
        nodes.append({
            "id": job_id,
            "type": "job",
...
def build_portfolio_map(profile: CareerProfile, store: JobsterStore) -> dict[str, Any]:
    ...
    return {
        ...
        "missing_proof": [
            ...
            if item["proof"] == "missing" and item["level"] in {"core", "strong", "working", "emerging"}
        ],
        "recurring_market_gaps": gaps[:10],
    }
...
def build_data_health(profile: CareerProfile, store: JobsterStore) -> dict[str, Any]:
    ...
    if not profile.portfolio_url:
        issues.append({"kind": "missing", "severity": "medium", "title": "Portfolio link is missing", "detail": "Jobster cannot connect portfolio proof to opportunities until a portfolio link is present."})
    if not profile.linkedin_url:
        issues.append({"kind": "missing", "severity": "low", "title": "LinkedIn link is missing", "detail": "This does not block applications, but it reduces profile completeness."})

    for capability in profile.capabilities:
        if capability.level.value in {"core", "strong"} and not capability.evidence_ids:
            issues.append({"kind": "evidence", "severity": "medium", "title": f"{capability.name} has no linked evidence", "detail": "A strong capability is more credible when it points to a real project, record, or artifact."})

    unresolved = [item for item in profile.evidence if item.status == "unresolved"]
    if unresolved:
        issues.append({"kind": "evidence", "severity": "medium", "title": f"{len(unresolved)} evidence item{'s' if len(unresolved) != 1 else ''} need review", "detail": "Unresolved evidence is kept separate so Jobster does not treat it as confirmed fact."})

    comp = profile.compensation
    if comp.minimum_monthly is not None and comp.target_monthly is not None and comp.minimum_monthly > comp.target_monthly:
        issues.append({"kind": "contradiction", "severity": "high", "title": "Pay settings conflict", "detail": "The minimum monthly pay is higher than the target monthly pay."})

    seen: set[tuple[str, str]] = set()
    for experience in profile.experiences:
        key = (experience.company.strip().lower(), experience.title.strip().lower())
        if key in seen:
            issues.append({"kind": "duplicate", "severity": "low", "title": f"Possible duplicate experience: {experience.company}", "detail": "Two experience entries use the same company and title. Check whether both are intentional."})
        seen.add(key)

    jobs = store.list_job_summaries(limit=500)
    unchecked = sum(1 for item in jobs if not item.get("verification_state"))
    if unchecked:
        issues.append({"kind": "freshness", "severity": "low", "title": f"{unchecked} job{'s' if unchecked != 1 else ''} have not been live-checked", "detail": "A source result can be real but already closed. Live-check strong jobs before applying."})

    weights = {"high": 18, "medium": 9, "low": 3}
    penalty = sum(weights.get(item["severity"], 4) for item in issues)
    score = max(0, 100 - min(100, penalty))
    return {"score": score, "status": "healthy" if score >= 85 else "review" if score >= 65 else "attention", "issues": issues[:40], "counts": dict(Counter(item["kind"] for item in issues))}
...
def build_learning_suggestions(profile: CareerProfile, store: JobsterStore) -> list[dict[str, Any]]:
    ...
    if reactions.get("outside_direction", 0) >= 3:
        suggestions.append({"id": "reduce-outside-direction", "kind": "preference", "title": "Tighten role direction", "detail": "You have marked several jobs as the wrong direction. Jobster can keep showing them for now, or you can tighten the target-role rules.", "evidence": f"{reactions['outside_direction']} explicit feedback events", "requires_approval": True})
    if reactions.get("salary_too_low", 0) >= 2:
        suggestions.append({"id": "review-minimum-pay", "kind": "compensation", "title": "Review your minimum pay", "detail": "You have repeatedly marked jobs as paying too little. Consider checking whether your minimum-pay rule still matches what you want.", "evidence": f"{reactions['salary_too_low']} pay-related feedback events", "requires_approval": True})

    for item in build_gap_signals(store)[:4]:
        if item.get("jobs", 0) >= 2:
            suggestions.append({"id": f"gap:{item['gap']}", "kind": "skill_gap", "title": f"Repeated gap: {item['gap']}", "detail": "This gap appears across multiple strong opportunities. It may be worth proving, learning, or explicitly deciding not to pursue.", "evidence": f"Appears in {item['jobs']} strong jobs", "requires_approval": True})

    if not suggestions:
        suggestions.append({"id": "not-enough-signal", "kind": "observation", "title": "No strong learning change yet", "detail": "Jobster is keeping your current career model stable because there is not enough repeated evidence to suggest a change.", "evidence": "Not enough repeated feedback yet", "requires_approval": False})
...
def build_decision_brief(profile: CareerProfile, store: JobsterStore, job_id: str) -> dict[str, Any] | None:
    ...
    headline = f"{title} at {company}" + (f" · {score}/100 match" if score is not None else "")
    return {
        "headline": headline,
